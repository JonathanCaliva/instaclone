...
async function isFollow(username,context){
    ...
    const follow = await Follow.find({idUser: context.user.id}).where('follow').equals(userFound._id)
...
async function unFollow(username,context){
    const userFound = await User.findOne({username})
    const follow = await Follow.deleteOne({idUser:context.user.id}).where('follow').equals(userFound._id)
